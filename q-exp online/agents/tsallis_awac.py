...
class TsallisAWAC(BaseAgent):
    ...
    def _exp_q(self, inputs):
        if self.entropic_index == 1:
            ...
            return torch.maximum(torch.FloatTensor([0.]), 1 + (1 - self.entropic_index) * inputs) ** (1/(1-self.entropic_index))
    ...
    def update_actor(self) -> None:
        ...
        # exp_q_scale = self._exp_q((best_q_values - best_q_values.max(dim=1, keepdim=True)[0]) / self.alpha)
        """
        ...
        when q - q.mean, both
        """
        # exp_q_scale = self._exp_q(best_q_values / self.alpha)
        ...
        exp_q_scale = self._exp_q((best_q_values - baseline) / self.alpha)
        policy_loss = - torch.mean(exp_q_scale * logprobs)
        # print(f"q values {best_q_values}, exp_q {exp_q_scale}, policy loss {policy_loss}")
